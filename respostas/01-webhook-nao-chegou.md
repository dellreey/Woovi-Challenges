...
1. Webhook configurado com sucesso para `OPENPIX:TRANSACTION_RECEIVED`: [captura](../evidencias/ticket-01/00-webhook-configurado.png).
2. Webhook de teste recebido com o evento configurado: [captura](../evidencias/ticket-01/01-webhook-teste.png).
3. Cobrança criada com sucesso no Sandbox: [captura](../evidencias/ticket-01/02-cobranca-criada.png).
4. Pagamento simulado e confirmado no Sandbox: [captura](../evidencias/ticket-01/03-pagamento-simulado.png).
5. Entrega do webhook `OPENPIX:TRANSACTION_RECEIVED` ao endpoint de teste: [captura](../evidencias/ticket-01/04-webhook-recebido.png).
